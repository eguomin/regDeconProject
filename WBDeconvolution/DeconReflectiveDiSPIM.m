% DeconReflectiveDiSPIM.m: spatial variant RL deconvolution for reflective diSPIM, compatible
...
N_Image = [ny, nx_img, nz_img];
nx_obj = max(nx_img, nz_img * interval);
nz_obj = nx_obj;
N_Obj = [ny, nx_obj, nz_obj];

[ny_PSF, nx_PSF, nz_PSF] = size(PSFA_fp);
px_range = (nx_PSF - max(N_Obj(2),256))/2+1 : (nx_PSF + max(N_Obj(2),256))/2;
py_range = (ny_PSF - max(N_Obj(1),256))/2+1:(max(N_Obj(1),256)+ny_PSF)/2;
pz_range = (nz_PSF - max(N_Obj(3),256))/2+1 : (nz_PSF + max(N_Obj(3),256))/2;
...
% excitation pattern
N_Exc = [ny, 3*nx_obj, 3*nz_obj];
mode = 0;
Excitation = IlluminationPattern(thickness,N_Exc,mode, pixel, angle);
...
for z = 1 : nz_img
        delta = (z - nz_img/2) * interval;
        if(gpuFlag)
            InterMediate = zeros([ny, nx_obj, nz_obj],'single', 'gpuArray');  
            InterMediate(:,:,delta + nz_obj/2) =  ones([ny,nx_obj],'single','gpuArray');
        else
            InterMediate = zeros([ny, nx_obj, nz_obj],'single');  
            InterMediate(:,:,delta + nz_obj/2) =  ones([ny,nx_obj],'single');
        end
        px_range = N_Exc(2)/2 - N_Obj(2)/2 + 1 - delta : N_Exc(2)/2 + N_Obj(2)/2 - delta;
        pz_range = N_Exc(3)/2 - N_Obj(3)/2 + 1 - delta : N_Exc(3)/2 + N_Obj(3)/2 - delta;  
        Sample_Illuminate = ConvFFT3_S(InterMediate,OTF_C).*Excitation(:,px_range, pz_range); 
        Sample_Illuminate = max(Sample_Illuminate, 0);
        SensA = SensA + single(Sample_Illuminate);
end
Sens = max(SensA, smallValue);
if(gpuFlag)
    WriteTifStack(gather(Sens),  [path_output, 'SensA.tif'], 32);
    disp(['Sensitivity Done! GPU free memory: ', num2str(g.FreeMemory/1024/1024), ' MB']);
else
    WriteTifStack(Sens,  [path_output, 'SensA.tif'], 32);
...
for iteration = 1:itNum
   ...
   if(gpuFlag)
        Correction = zeros([ny, nx_obj, nz_obj],'single', 'gpuArray'); 
   else
        Correction = zeros([ny, nx_obj, nz_obj],'single');   
   end
   for z = 1 : nz_img  
        delta = (z - nz_img/2) * interval;         
        px_range = N_Exc(2)/2 - N_Obj(2)/2 + 1 - delta : N_Exc(2)/2 + N_Obj(2)/2 - delta;
        pz_range = N_Exc(3)/2 - N_Obj(3)/2 + 1 - delta : N_Exc(3)/2 + N_Obj(3)/2 - delta;    
        Sample_Blur = ConvFFT3_S(Estimate.*Excitation(:,px_range, pz_range), OTF_A);
        if(gpuFlag)
            InterMediate = zeros([ny, nx_obj, nz_obj],'single', 'gpuArray'); 
        else
            InterMediate = zeros([ny, nx_obj, nz_obj],'single'); 
        end
        InterMediate(:,:,delta + nz_obj/2) = ViewA(:,:,z)./Sample_Blur(:,:,delta + N_Obj(3)/2);       
        Sample_Illuminate = ConvFFT3_S(InterMediate,OTF_C).*Excitation(:,px_range,pz_range); 
        Correction = Correction + Sample_Illuminate;
    end
        
    Estimate = Estimate.* Correction./Sens; 
    ...
    for z = 1 : nz_img
        ...
        px_range = N_Exc(2)/2 - N_Obj(2)/2 + 1 - delta : N_Exc(2)/2 + N_Obj(2)/2 - delta;
        pz_range = N_Exc(3)/2 - N_Obj(3)/2 + 1 - delta : N_Exc(3)/2 + N_Obj(3)/2 - delta;                    
        Sample_Blur = ConvFFT3_S(Estimate.*Excitation(:,px_range, pz_range), OTF_B);
        if(gpuFlag)
            InterMediate = zeros([ny, nx_obj, nz_obj],'single', 'gpuArray');  
        else
            InterMediate = zeros([ny, nx_obj, nz_obj],'single');  
        end
        InterMediate(:,delta + nx_obj/2,:) = squeeze(ViewB(:,z,:))./squeeze(Sample_Blur(:,delta + nx_obj/2,:));       
        Sample_Illuminate = ConvFFT3_S(InterMediate,OTF_D).*Excitation(:,px_range, pz_range); 
        Correction = Correction + Sample_Illuminate;
    end
    Estimate = Estimate.* Correction./permute(Sens,[1,3,2]);
    Estimate = max(Estimate, smallValue);
